...
* The Aircraft Engine Analysis Tool (AEAT) can be used to visualize a variety of aircraft age, engine type, and ownership metrics across sub-fleets, airlines, or the entire US air transportation system. 
...
6: [ Aircraft Assignment Analysis Tool (AAAT) ](#AAAT) <br />
7: [ Aircraft Engine Analysis Tool (AEAT) ](#AEAT) <br />
...
**Tool Purpose:** ```ATAT``` can be used to to visualize longitudinal trends (i.e., year-by-year) on the outputs generated from using APAT and AFAT. Sample plots generated by the tool can be seen in **Figure 1 - Figure 2**.
...
**User Actions:** ```ATAT``` can be run as a MATLAB script, with minimal user action. The user can modify some of the fields under the "USER DEFINED INPUTS" section of ```ATAT``` to generate custom plots. For example:
...
**Tool Purpose:** ```AEAT``` can be used to to visualize a variety of aircraft age, engine type, and ownership metrics across sub-fleets, airlines, or the entire US air transportation system. Sample plots generated by the tool can be seen in **Figure 3 - Figure 5**.
...
* Setting ```Desired_Aircraft = {'A319-100','A320-200','A321-200'}``` and ```Desired_Airline = {}``` will result in **Figure 3**, which displays metrics specific to the A320 family, across all US airlines.
...
**Figure 3:** _A320 Family Age and Ownership Metrics (as of June 2026)_.
</p>

* Setting ```Desired_Aircraft = {'A319-100','A320-200','A321-200'}``` and ```Desired_Airline = {'AMERICAN AIRLINES INC'}``` will result in **Figure 4**, which displays the same information as before, but specific to American. The difference is in the lower bottom plot, which now displays the number of aircraft of the A320 family within American's fleet.
...
**Figure 4:** _A320 Family Age and Ownership Metrics specific for American (as of June 2026)_.
</p>

* Setting ```Desired_Aircraft = {}``` and ```Desired_Airline = {'AMERICAN AIRLINES INC'}``` will result in **Figure 5**, which displays fleet-wide information specific to American. 
...
**Figure 5:** _A320 Family Age and Ownership Metrics specific for American (as of June 2026)_.
</p>
